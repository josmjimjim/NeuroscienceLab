...
class DisplayCSVData(QWidget):
    ...
    def setupModelView(self):
        ...
        emotions = ('0', 'angry', 'disgust', 'fear', 'happy', 'sad',
                    'surprise', 'neutral')
        try:
        ...
        if self.trigger_sig and shape == 9:
            emotions = emotions + ('trigger',)
            self.displayTable(file_name, emotions)
    ...
    def displayTable (self, file_name, emotions):
        file_name.columns = list(emotions)
        file_name = file_name.drop('0', axis=1)
        # Set up standard item model and table view.
        table_view = QTableView()
        # From QAbstractItemView.ExtendedSelection = 3
        table_view.SelectionMode(3)
        self.model = BuildModel(file_name, emotions, self.trigger_sig, self)
